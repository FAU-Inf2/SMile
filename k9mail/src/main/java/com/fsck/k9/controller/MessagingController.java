...
public class MessagingController implements Runnable {
    ...
    private final BlockingQueue<Command> mCommands; //Worklist
    private final Thread mThread;
    private final Set<MessagingListener> mListeners;
    private final ConcurrentHashMap<String, AtomicInteger> sendCount;
    private final ConcurrentHashMap<Account, Pusher> pushers;
    private final ExecutorService threadPool;
    private final MemorizingListener memorizingListener;
    ...
    private MessagingController(Context context) {
        ...
        sendCount = new ConcurrentHashMap<>();
        memorizingListener = new MemorizingListener();
        threadPool = Executors.newCachedThreadPool();
        pushers = new ConcurrentHashMap<>();
        notificationController = NotificationController.newInstance(context);
        ...
        while (true) {
            ...
            try {
                ...
                if (command != null) {
                    ...
                    if (K9.DEBUG)
                        Log.i(K9.LOG_TAG, "Running " + (command.isForeground() ? "Foreground" : "Background") + " command '" + command.getDescription() + "', seq = " + command.getSequence());
                    ...
                    if (K9.DEBUG)
                        Log.i(K9.LOG_TAG, (command.isForeground() ? "Foreground" : "Background") +
                                " Command '" + command.getDescription() + "' completed");

                    for (MessagingListener l : getListeners(command.getListener())) {
    ...
    private void putCommand(BlockingQueue<Command> queue, String description, MessagingListener listener, Runnable runnable, boolean isForeground) {
        ...
        while (retries-- > 0) {
            try {
                Command command = new Command(runnable, listener, description, isForeground, sequencing.getAndIncrement());
                queue.put(command);
                return;
    ...
    public void searchLocalMessages(final LocalSearch search, final MessagingListener listener) {
        ...
        threadPool.execute(new SearchLocalMessagesRunnable(accounts, search, listener));
    }

    /**
     * Find messages in the given account matching the query.
     *
     * @param acctUuid       The account to search in.
    ...
    public Future<?> searchRemoteMessages(final String acctUuid, final String folderName, final String query,
        ...
        return threadPool.submit(new SearchRemoteMessagesRunnable(acctUuid, folderName, query, requiredFlags, forbiddenFlags, listener));
    }

    /**
     * Fetches messages from local and remote folders.
     *
     * @param account    The account to fetch messages from.
     * @param folderName The folder containing the messages.
     * @param messages   List of Messages to fetch.
     * @param listener   Listener to notify.
     */
    public void loadSearchResults(final Account account, final String folderName, final List<Message> messages, final MessagingListener listener) {
        threadPool.execute(new LoadSearchResultsRunnable(listener, account, folderName, messages));
    }

    /**
     * Fetches messages from local folder. If the message is not stored locally it is fetched from the remote folder.
     *
     * @param messages     List of messages to fetch.
     * @param localFolder  The local folder which may contains the messages.
     * @param remoteFolder The remote folder to fetch messages from, if they are not stored locally.
     * @param listener     The listener to notify.
     * @throws MessagingException
     */
    ...
    /**
     * Load more messages from the given account.
     *
     * @param account  The account containing the folder.
     * @param folder   The folder for which more messages should be loaded.
     * @param listener The listener to notify.
     */
    ...
    public void synchronizeMailbox(final Account account, final String folder, final MessagingListener listener, final Folder providedRemoteFolder) {
        putBackground("synchronizeMailbox", listener, new SynchronizeMailboxRunnable(account, folder, listener, providedRemoteFolder));
    }
    ...
    private void synchronizeMailboxSynchronous(final Account account, final String folder, final MessagingListener listener, Folder providedRemoteFolder) {
        ...
        try {
            ...
            remoteFolder = ensureRemoteFolder(account, folder, listener, providedRemoteFolder);
            ...
            int remoteStart = syncGetNewMessages(account, folder, listener, remoteFolder, localUidMap, remoteMessageCount, visibleLimit, remoteMessages, remoteUidMap, earliestDate);
            MoreMessages moreMessages = syncRemoveRemoteDeletedMessages(account, listener, localFolder, localMessages, remoteUidMap);
            ...
            syncNotifyListeners(account, folder, listener, tLocalFolder, commandException, localFolder, remoteMessageCount, newMessages);
        } catch (Exception e) {
    ...
    private void updateMoreMessages(Folder remoteFolder, LocalFolder localFolder, Date earliestDate, int remoteStart)
        ...
        if (remoteStart == 1) {
            ...
            LocalFolder.MoreMessages newMoreMessages = (moreMessagesAvailable) ? MoreMessages.TRUE : MoreMessages.FALSE;
    ...
    private void syncNotifyListeners(Account account, String folder, MessagingListener listener, LocalFolder tLocalFolder, Exception commandException, LocalFolder localFolder, int remoteMessageCount, int newMessages) throws MessagingException {
    ...
    private MoreMessages syncRemoveRemoteDeletedMessages(Account account, MessagingListener listener,
                                                 LocalFolder localFolder,
                                                 List<? extends Message> localMessages,
                                                 Map<String, Message> remoteUidMap) {
        /*
        ...
        if(!destroyMessages.isEmpty()) {
            ...
            for (Message destroyMessage : destroyMessages) {
                for (MessagingListener l : getListeners(listener)) {
                    l.synchronizeMailboxRemovedMessage(account, localFolder.getName(), destroyMessage);
                }
            }
        }
    ...
    private int syncGetNewMessages(Account account, String folder, MessagingListener listener,
                                    Folder remoteFolder, Map<String, Message> localUidMap,
                                    int remoteMessageCount, int visibleLimit,
                                    List<Message> remoteMessages, Map<String, Message> remoteUidMap,
                                    Date earliestDate) throws MessagingException {
        if (remoteMessageCount <= 0) {
            throw new MessagingException("Message count " + remoteMessageCount + " for folder " + folder);
        }
    ...
    private void refreshLocalMessageFlags(final Account account, final Folder remoteFolder,
        ...
        int unreadBeforeStart = 0;
        try {
            AccountStats accountStats = account.getStats(context);
            unreadBeforeStart = accountStats.unreadMessageCount;
        } catch (MessagingException ex) {
            Log.e(K9.LOG_TAG, "Unable to retrieve unreadMessageCount", ex);
        }
    ...
    /**
     * Add an error message to the given account.
     *
     * @param account The account to add the error message to.
    ...
    /**
     * Create a task to mark all messages as read.
     *
     * @param account The account containing the folder.
     * @param folder  The folder containing messages to mark.
     */
    ...
    /**
     * Create a task to set a flag to a list of messages.
     *
     * @param account    The account containing the messages.
     * @param messageIds The ids of the messages where the flag should be set.
    ...
    /**
     * Add a task to set flags for a message thead.
     *
     * @param account       The account containing the thread.
     * @param threadRootIds The id of the root message.
     * @param flag          The flag to set.
     * @param newState      The state to set the flag to.
     */
     ...
     *
     * @param account  Account containing the folder.
     * @param folder   The folder containing the message.
     * @param uid      The uid of the message to download.
     * @param listener The listener to notify.
     */
    ...
    /**
     * Get the message to display it.
     *
     * @param account  The account containing the folder.
     * @param folder   The folder containing the message.
     * @param uid      The uid of the message to load.
     * @param listener The listener to notify.
     */
    ...
    /**
     * Returns the local message with the given uid.
     *
     * @param account    The account containing the folder.
     * @param folderName The folder containing the message.
     * @param uid        The uid of the message.
     * @return A local message with the given uid.
     * @throws MessagingException
     */
    ...
    /**
     * Add a command to load an attachment.
     *
     * @param account  The account the message belongs to.
     * @param message  The message with the attachment.
     * @param part     The part of the attachment that should be loaded.
     * @param listener The listener to notify.
     */
     ...
     * Delete a draft.
     *
     * @param account The account the draft belongs to.
     ...
     *
     * @param messages The messages which threads should be deleted.
     */
    public void deleteThreads(final List<LocalMessage> messages) {
        actOnMessages(messages, new MyMessageActor(messages));
    }
    ...
    /**
     * Return a list of messages belonging to a message thread.
     *
     * @param account  The account the messages belong to.
     * @param messages The the messages whose thrads should be collected.
     * @return A list of messages belonging to threads.
     * @throws MessagingException
     */
    ...
    /**
     * Add a command to delete messages.
     *
    ...
    /**
     * Add a command to empty the trash.
     *
     * @param account  The account whose trash should be deleted.
     * @param listener The listener to notify.
     */
    ...
    /**
     * Add a command to send a message with an alternative program.
     *
    ...
    private void synchronizeFolder(
        ...
        final long nextSyncTime = System.currentTimeMillis() - accountInterval;
        if (!ignoreLastCheckedTime && folder.getLastChecked() > nextSyncTime) {
            if (K9.DEBUG) {
    ...
    /**
     * Add a command to compress message storage.
     *
     * @param account The account to compress space.
     * @param ml      The listener to notify.
     */
    ...
    /**
     * Delete localy stored mails.
     *
     * @param account The account whose messages should be cleared.
     * @param ml      The listener to notify.
     */
     ...
     *
     * @param account The account whose database should be recreated.
     * @param ml      The listener to notify
     */
     ...
     *
     * @param message The message whose uid should be returned.
     * @return The uid of the given message.
     */
    ...
    /**
     * Setup pushing service for a given account.
     */
    ...
    /**
     * Add a background command to download new messages.
     */
    public void messagesArrived(final Account account, final Folder remoteFolder, final List<Message> messages, final boolean flagSyncOnly) {
        ...
        putBackground("Push messageArrived of account " + account.getDescription()
                + ", folder " + remoteFolder.getName(), null, new MessagesArrivedRunnable(account, remoteFolder, messages, flagSyncOnly, latch));
    ...
    private class RefreshRemoteRunnable implements Runnable {
        private final Account account;
        private final MessagingListener listener;

        public RefreshRemoteRunnable(Account account, MessagingListener listener) {
    ...
    private class SearchLocalMessagesRunnable implements Runnable {
        private final LocalSearch search;
        private final MessagingListener listener;
        private final List<Account> accounts;

        public SearchLocalMessagesRunnable(List<Account> accounts, LocalSearch search, MessagingListener listener) {
            this.search = search;
            this.listener = listener;
        ...
        private void searchLocalMessagesSynchronous(final LocalSearch search, final MessagingListener listener) {
            ...
            for (final Account account : accounts) {
                ...
                MessageRetrievalListener retrievalListener = new LocalMessageMessageRetrievalListener(stats, listener, account);
    ...
    private class MyMessageActor implements MessageActor {

        private final List<LocalMessage> messages;

        public MyMessageActor(List<LocalMessage> messages) {
        ...
        public void act(final Account account, final Folder folder,
            ...
            putBackground("deleteThreads", null, new DeleteThreadRunnable(account, folder, accountMessages));
        }

    }

    private class DeleteThreadRunnable implements Runnable {
        private final Account account;
        private final Folder folder;
        private final List<Message> accountMessages;

        public DeleteThreadRunnable(Account account, Folder folder, List<Message> accountMessages) {
            this.account = account;
            this.folder = folder;
    ...
    private class MessagesArrivedRunnable implements Runnable {
        private final Account account;
        private final Folder remoteFolder;
        private final List<Message> messages;
        private final boolean flagSyncOnly;
        private final CountDownLatch latch;

        public MessagesArrivedRunnable(Account account, Folder remoteFolder, List<Message> messages, boolean flagSyncOnly, CountDownLatch latch) {
            this.account = account;
            this.remoteFolder = remoteFolder;
            this.messages = messages;
            this.flagSyncOnly = flagSyncOnly;
            this.latch = latch;
        }
    ...
    private class SearchRemoteMessagesRunnable implements Runnable {
        ...
        public SearchRemoteMessagesRunnable(String acctUuid, String folderName, String query, Set<Flag> requiredFlags, Set<Flag> forbiddenFlags, MessagingListener listener) {
    ...
    private class LoadSearchResultsRunnable implements Runnable {
        private final MessagingListener listener;
        private final Account account;
        private final String folderName;
        private final List<Message> messages;

        public LoadSearchResultsRunnable(MessagingListener listener, Account account, String folderName, List<Message> messages) {
            this.listener = listener;
            this.account = account;
            this.folderName = folderName;
            this.messages = messages;
    ...
    private class SynchronizeFolderRunnable implements Runnable {
        private final Account account;
        private final Folder folder;
        private final boolean ignoreLastCheckedTime;
        private final long accountInterval;
        private final MessagingListener listener;

        public SynchronizeFolderRunnable(Account account, Folder folder, boolean ignoreLastCheckedTime, long accountInterval, MessagingListener listener) {
            this.account = account;
            this.folder = folder;
            this.ignoreLastCheckedTime = ignoreLastCheckedTime;
            this.accountInterval = accountInterval;
            this.listener = listener;
        }
    ...
    private class SynchronizeMailboxRunnable implements Runnable {
        private final Account account;
        private final String folder;
        private final MessagingListener listener;
        private final Folder providedRemoteFolder;

        public SynchronizeMailboxRunnable(Account account, String folder, MessagingListener listener, Folder providedRemoteFolder) {
            this.account = account;
            this.folder = folder;
            this.listener = listener;
            this.providedRemoteFolder = providedRemoteFolder;
        }
    ...
    private class SmallMessageRetrievalListener<T extends Message> implements MessageRetrievalListener<T> {
        private final Account account;
        private final String folder;
        ...
        public SmallMessageRetrievalListener(Account account, String folder, AtomicInteger progress, Date earliestDate, LocalFolder localFolder, AtomicInteger newMessages, int todo, int unreadBeforeStart) {
            this.account = account;
            this.folder = folder;
